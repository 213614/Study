...
i:\java202207\database\20220823_01_emp 테이블 문제.sql
////////////////////////////////////////////////////////////////////////////////
drop table emp;
...
select trunc((sysdate-hiredate)/365,0) from emp where ename='손흥민';       -- 20
-- 3) 입사 동기찾기 
select ename, hiredate, trunc((sysdate-hiredate)/365,0) as 근속
from emp
...
-- 강사님 코드 : 산술식에 계속 쓰이니까 comm 에 default 0 을 아예 깔고 할듯 (null값은 계산안됨)
-- 1) 근속년수 15년 이상만 조회
select ename, hiredate, sal
from emp
where trunc((sysdate-hiredate)/365,0) >= 15;
-- 2) 현재 연봉 (커미션 null은 0으로 대체)
...
from emp
where trunc((sysdate-hiredate)/365,0) >= 15;
-- 4) 최종 연봉 반올림해서 소수점 없애기 
select ename, hiredate, sal
     , sal*12+nvl(comm,0)                                          as 현재연봉 
     , (sal*12+nvl(comm,0))*0.1                                    as 인상된금액
     , round((sal*12+nvl(comm,0)) + ((sal*12+nvl(comm,0))*0.1))    as 최종연봉
from emp
where trunc((sysdate-hiredate)/365,0) >= 15;
-- 5) 고액 연봉순으로 정렬
select ename, hiredate, sal
     , trunc((sysdate-hiredate)/365,0)                             as 근속년수
     , sal*12+nvl(comm,0)                                          as 현재연봉 
     , (sal*12+nvl(comm,0))*0.1                                    as 인상된금액
     , round((sal*12+nvl(comm,0)) + ((sal*12+nvl(comm,0))*0.1))    as 최종연봉
from emp
where trunc((sysdate-hiredate)/365,0) >= 15
order by 최종연봉 desc;     -- as로 써도 먹힘
...
group by job
order by avg(sal);
--→ 동일 값은 나오지만 to_char은 형변환하는 것(hiredate의 연도를 문자열데이터로 변환)이라 여기선 extract를 써야 더 알맞다 
 
-- 강사님 코드
-- 1) 직급, 입사일 조회
select job, hiredate from emp;
-- 2) 입사일에서 연도만 추출
select hiredate, extract(year from hiredate) from emp;      -- 2000/2001처럼 완전히 나옴 to_char는 뒤에만 나옴 00/01
-- 3) 짝수년도 추출 
select job, hiredate ,extract(year from hiredate) as even
from emp 
